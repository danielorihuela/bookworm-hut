...
* [`src/bookworm_hut_frontend/`](src/bookworm_hut_frontend/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/bookworm_hut_frontend/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `bookworm-hut-frontend` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/bookworm_hut_frontend/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns bookworm-hut-frontend.example
  (:require [bookworm-hut-frontend.config :as config])
